# =============================================================================
# model.R  --  Daten, Vorverarbeitung und DLNM-Engine
#
# WICHTIG: Diese Datei laeuft NICHT in der deployten App. Sie braucht den
# Originaldatensatz und wird ausschliesslich offline von build.R benutzt, um
# data/app_data.rds zu erzeugen. Die App selbst (global.R) laedt nur dieses
# fertige Ergebnis -- der Datensatz wird bewusst nicht mitveroeffentlicht.
#
# Enthaelt die komplette Modelllogik aus DLNM_Masterarbeit_Alfter.R, gekapselt
# so dass ein einzelnes Outcome geschaetzt werden kann (ein GLM ist < 1 s).
# =============================================================================
...
# -----------------------------------------------------------------------------
# Der Datensatz liegt bewusst AUSSERHALB des (oeffentlichen) Repos. Ueber die
# Umgebungsvariable THESIS_DATA laesst sich ein anderer Pfad setzen.
DATA_PATH <- Sys.getenv(
  "THESIS_DATA",
  "C:/Users/chris/OneDrive/Masterarbeit/Abgabe_Masterarbeit_Alfter/Datensatz_Masterarbeit_Alfter.xlsx"
)
if (!file.exists(DATA_PATH))
  stop("Datensatz nicht gefunden: ", DATA_PATH,
       "\nPfad ueber die Umgebungsvariable THESIS_DATA setzen.")
...
# Monatsaggregat fuer den Data-Tab. Die App zeigt bewusst KEINE Tageswerte mehr:
# eine tagesgenaue Zeitreihe waere faktisch der Rohdatensatz in Kurvenform.
# Faelle werden je Monat summiert, Temperaturen gemittelt.
monthly_series <- function() {
  count_cols <- c("gesamt", outcome_meta$key[outcome_meta$key != "gesamt"])
  count_cols <- intersect(count_cols, names(df))
  out <- df %>%
    mutate(month = as.Date(format(datum, "%Y-%m-01"))) %>%
    group_by(month) %>%
    summarise(
      year         = dplyr::first(year),
      days         = dplyr::n(),
      lufttemp_avg = mean(lufttemp_avg, na.rm = TRUE),
      tautemp_avg  = mean(tautemp_avg,  na.rm = TRUE),
