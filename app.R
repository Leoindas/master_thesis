      ...
      card(card_body(class = "p-4",
        ...
        accordion(
          ...
          accordion_panel(
            ...
            p(strong("What is a DLNM?"), " A Distributed-Lag Non-Linear Model is a ",
              "regression framework developed for environmental time series ",
              "(Gasparrini et al., 2010). It captures two things at once within a ",
              "single model: the ", strong("non-linear"), " relationship between ",
              "temperature and health, and the ", strong("delayed (lagged)"),
              " effects that unfold over the days after exposure."),
            p("Extreme temperatures rarely act only on the same day – cold effects in ",
              "particular build up with a delay and persist for days, while heat tends ",
              "to strike almost immediately. At the same time the temperature–risk ",
              "relationship is non-linear: both very high and very low temperatures ",
              "raise the risk of admission, whereas moderate temperatures show almost ",
              "none. This produces the characteristic ", strong("U- or J-shaped"),
              " risk curve seen in the Dose-response tab."),
            p("The DLNM encodes both dimensions in a so-called ", strong("cross-basis"),
              " – a two-dimensional predictor that combines a spline over temperature ",
              "with a spline over the lag. Because this cross-basis simply enters an ",
              "ordinary GLM, the model stays linear in its parameters even though the ",
              "temperature–health surface it describes is non-linear."),
            tags$hr(),
